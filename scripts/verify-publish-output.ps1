...
$executablePath = Join-Path $directory.FullName 'Otium.exe'
...
if ($header[0] -ne 0x4D -or $header[1] -ne 0x5A) {
    throw 'Published Otium.exe is not a valid Windows PE file.'
}

$info = [System.Diagnostics.FileVersionInfo]::GetVersionInfo($executable.FullName)
if ($info.ProductName -ne 'Otium') {
...
$unexpectedFiles = @(Get-ChildItem -LiteralPath $directory.FullName -File | Where-Object { $_.Name -ne 'Otium.exe' })
