...
{
    private const string PayloadResourceName = "Kvieta.Payload.msi";
        ...
        bool forceReinstall)
    {
        ...
        try
        {
            ...
            string msiPath = Path.Combine(stagingDirectory, "Kvieta.msi");
            ...
            string logPath = Path.Combine(logDirectory, $"Kvieta-setup-{DateTime.UtcNow:yyyyMMdd-HHmmss}.log");
            string features = desktopShortcut ? "MainFeature,DesktopShortcutFeature" : "MainFeature";

            // Early alpha builds exposed uninstall only through Kvieta.exe. If that
    ...
    private static bool StopGuardianForUpgrade()
    {
        try
        {
            using ServiceController controller = new("KvietaGuardian");
            controller.Refresh();
    ...
    private static bool TryRemoveOrphanedGuardianService(string logPath)
    {
        ...
        AppendRecoveryLog(logPath,
            "Guardian service is registered but its Kvieta executable is missing. Removing the orphaned service before MSI installation.");
        ...
        try
        {
            ...
            deleteInfo.ArgumentList.Add("KvietaGuardian");
    ...
    private static void StopInstalledKvietaProcesses()
    {
        string installedExecutable = Path.GetFullPath(Path.Combine(
            Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles),
            "Kvieta",
            "Kvieta.exe"));
        foreach (Process process in Process.GetProcessesByName("Kvieta"))
        {
            using (process)
            {
                try
                {
                    ...
                    if (!process.WaitForExit(8000))
                    {
                        throw new InvalidOperationException(
                            $"Installed Kvieta process {process.Id} did not stop before the upgrade.");
                    }
                }
