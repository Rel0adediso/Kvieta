...
internal sealed class ResilientJsonFile<T> where T : class
{
    ...
    public async Task<T> LoadReadOnlyAsync(CancellationToken cancellationToken = default)
    {
        ...
        catch (Exception primaryException) when (IsRecoverableReadFailure(primaryException))
        {
            ...
            {
                throw new InvalidDataException($"Kvieta veri dosyası okunamadı ve sağlam yedek bulunamadı: {FilePath}", primaryException);
            }
    ...
    public async Task<T> UpdateAsync(Func<T, T> update, CancellationToken cancellationToken = default)
    {
        ...
        T updated = update(current) ?? throw new InvalidDataException("Kvieta veri güncellemesi geçersiz sonuç üretti.");
        ...
        catch (Exception primaryException) when (IsRecoverableReadFailure(primaryException))
        {
            ...
            catch (Exception backupException) when (IsRecoverableReadFailure(backupException))
            {
                throw new InvalidDataException(
                    $"Kvieta veri dosyası ve son sağlam yedeği okunamadı: {FilePath}",
    ...
    public async Task<T> RestoreBackupAsync(CancellationToken cancellationToken = default)
    {
        ...
        if (!File.Exists(BackupPath))
        {
            throw new FileNotFoundException("Kvieta son sağlam yedeği bulunamadı.", BackupPath);
        }
    ...
    private async Task<MigrationResult<T>> ReadAndMigrateAsync(string path, CancellationToken cancellationToken)
    {
        ...
        if (value is null)
        {
            throw new InvalidDataException($"Kvieta veri dosyası boş veya geçersiz: {path}");
        }
