...
public sealed class KvietaGuardianService : ServiceBase
{
    private const uint InvalidSessionId = 0xFFFFFFFF;
    private CancellationTokenSource? _cancellation;
    private Task? _worker;

    public KvietaGuardianService()
    {
    ...
    private static bool ShouldDeferForPostInstallControlCenter(int sessionId)
    {
        ...
        bool controlCenterProcessAlive = Process.GetProcessesByName("Kvieta").Any(process =>
        {
            using (process)
            {
                try
