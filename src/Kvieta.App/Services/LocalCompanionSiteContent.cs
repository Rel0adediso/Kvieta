            ...
            <head>
              ...
              <style>
                ...
                *{box-sizing:border-box}body{margin:0;min-height:100svh;background:#171813}
                ...
                .brand{display:flex;align-items:center;gap:10px;color:#f1eee3;font-size:13px;font-weight:650}.brand::after{content:"Yerel yönetici onayı";margin-left:auto;color:#949184;font-size:11px;font-weight:500}.mark{width:29px;height:29px;padding:3px;border-radius:8px;background:#f2eee3;box-shadow:0 2px 8px #0e0f0b40}.mark svg{display:block;width:100%;height:100%}
                ...
                .compare{margin:24px 0 16px;padding:17px;border:1px solid #7d806f;border-radius:10px;background:#1f201a;text-align:center}.compare small{display:block;color:#b0ac9f;font-size:11px;font-weight:600;letter-spacing:.055em;text-transform:uppercase}.code{margin-top:7px;color:#b4bc82;font-family:"Segoe UI Variable Display","Segoe UI",system-ui,sans-serif;font-size:37px;font-weight:650;letter-spacing:.15em;font-variant-numeric:tabular-nums}
                ...
                .actions{display:grid;gap:9px}button{min-height:46px;border-radius:7px;padding:0 16px;font:inherit;font-weight:650;cursor:pointer}button:focus-visible{outline:2px solid #b4bc82;outline-offset:3px}button:disabled{opacity:.42;cursor:default}.primary{border:0;color:#202217;background:#b4bc82}.primary:hover{background:#c7ce96}.primary:active{opacity:.82}.secondary{color:#f1eee3;background:#282a23;border:1px solid #7d806f}.secondary:hover{background:#303229}
                .result{margin-top:14px;padding:11px 12px;border-radius:8px;font-size:13px;line-height:1.45}.result:empty{display:none}.success{color:#9abc80;background:#293523}.error{color:#d98080;background:#382322}.loading{text-align:center;color:#b4bc82;font-size:13px}.loading.error{color:#d98080}
                @media(prefers-color-scheme:light){:root{color:#22231d;background:#d8d6cc;color-scheme:light}body{background:#d8d6cc}body::before{background:#4e5a35}.card{color:#22231d;background:#f2eee3;border-color:#bdbcb0;box-shadow:0 18px 45px #64625830}.brand{color:#22231d}.brand::after{color:#606257}p{color:#505248}.compare{background:#eae6dc;border-color:#777a6c}.compare small{color:#505248}.code{color:#4e5a35}.notice{background:#e3dfd5;border-color:#777a6c;color:#505248}.notice::before{border-color:#777a6c;color:#4e5a35}.primary{color:#f6f5ec;background:#4e5a35}.primary:hover{background:#46522f}.secondary{color:#22231d;background:#dedbd1;border-color:#777a6c}.secondary:hover{background:#d2cfc4}button:focus-visible{outline-color:#4e5a35}.success{color:#4e6b3e;background:#cfdcc7}.error{color:#973f3f;background:#ead0cd}.loading{color:#4e5a35}.loading.error{color:#973f3f} }
                [hidden]{display:none!important}
              </style>
            </head>
            <body>
              <main>
                <div id="loading" class="loading" role="status" aria-live="polite" aria-atomic="true">Kvieta bilgisayarına bağlanıyor…</div>
